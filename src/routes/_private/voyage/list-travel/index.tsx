...
function ListTravelPage() {
  ...
  const { data: rawData, isLoading } = useVoyages({
    ...searchParams,
    idEnterprise,
  ...
  const totalPages = Math.ceil(totalCount / searchParams.size);
  const kpiData = data?.analytics || [];
    ...
    <Card>
      <CardHeader title={t('voyage.list')}>
        <div className="flex items-center gap-2">
          <Button onClick={() => navigate({ to: '/voyage/list-travel/add' })}>
      ...
      {totalCount > 0 && (
        <CardFooter layout="multi">
          <div className="order-2 flex items-center gap-2 text-muted-foreground text-sm sm:order-1">
            <span>{t('show')}</span>
            <Select value={String(searchParams.size)} onValueChange={(val) => navigate({ search: (prev) => ({ ...prev, size: Number(val), page: 1 }) })}>
              ...
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      if (searchParams.page > 1) navigate({ search: (prev: VoyageSearch) => ({ ...prev, page: searchParams.page - 1 }) });
                    }}
                    aria-disabled={searchParams.page <= 1}
                  ...
                  return (
                    <PaginationItem key={pageNum}>
                      <PaginationLink
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          navigate({ search: (prev: VoyageSearch) => ({ ...prev, page: pageNum }) });
                        }}
                        isActive={searchParams.page === pageNum}
                ...
                <PaginationItem>
                  <PaginationNext
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      if (searchParams.page < totalPages) navigate({ search: (prev: VoyageSearch) => ({ ...prev, page: searchParams.page + 1 }) });
                    }}
                    aria-disabled={searchParams.page >= totalPages}
