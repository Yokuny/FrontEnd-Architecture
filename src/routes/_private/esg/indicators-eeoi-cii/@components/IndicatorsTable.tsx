...
export function IndicatorsTable({ data }: IndicatorsTableProps) {
  ...
  const columns: DataTableColumn<EEOICIIIndicator>[] = [
    ...
    },
    {
      ...
      render: (_, row) => (row.timeInVoyage > 0 ? (row.distanceInVoyage / row.timeInVoyage).toFixed(1) : '0.0'),
    },
    {
      key: 'consumption',
      header: `${t('consume')} (Ton)`,
      render: (value) => {
