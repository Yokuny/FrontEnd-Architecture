import type { EquipmentType, HeatmapFleetItem, SubgroupData } from '../@interface/heatmap.types';
...
export const EQUIPMENT_TYPES: EquipmentType[] = [
  ...
  },
  {
    ...
    name: 'PTO',
    code: 'pto',
    subgroups: [{ subgroupName: 'PTO 1' }, { subgroupName: 'PTO 2' }],
    options: ['1', '2'],
  },
];

const hasValidSubgroupData = (subgroup: SubgroupData): boolean => {
  const hasValidSensorOnOff = subgroup.idSensorOnOff && typeof subgroup.idSensorOnOff === 'string' && subgroup.idSensorOnOff.trim() !== '';
...
export const getAvailableEquipments = (fleetData: HeatmapFleetItem[]): EquipmentType[] => {
  const equipmentsWithData = new Set<string>();
  ...
  fleetData?.forEach((fleet) => {
    fleet.equipments?.forEach((equipment) => {
      ...
      const hasValidData = Array.isArray(equipment.subgroups) && equipment.subgroups.some((subgroup) => hasValidSubgroupData(subgroup));
  ...
  return EQUIPMENT_TYPES.filter((eqp) => equipmentsWithData.has(eqp.code));
};
