...
export function StatusChart({ filters }: { filters: KPISCMMSFilters & { min: string; max: string } }) {
  ...
  const { chartData, chartConfig, totalValue } = useMemo(() => {
    ...
    const openCount = data.filter((x) => !x.dataConclusao).length;
    const closedCount = data.filter((x) => x.dataConclusao).length;
  ...
  return (
    <Item variant="outline" className="flex-col items-stretch w-full flex-1">
      <ItemHeader className="items-center flex-col pb-4">
        <ItemTitle className="text-lg font-bold">{t('kpis.cmms.tasks.open.vs.closed')}</ItemTitle>
