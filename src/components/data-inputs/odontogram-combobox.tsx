...
import { useOdontogramsComboboxQuery } from '@/query/odontogram';
...
const OdontogramCombobox = ({ value, onChange, patient, disabled }: OdontogramComboboxProps) => {
  const [open, setOpen] = useState(false);
  const { options, isLoading } = useOdontogramsComboboxQuery(patient);
      ...
      <PopoverTrigger asChild>
        <Button role="combobox" variant="outline" aria-expanded={open} disabled={disabled || isLoading}>
          <div className="flex items-center gap-2 truncate">
            <span className="text-foreground/90">{selected?.label || t('odontograms')}</span>
          </div>
            ...
            {options.map((item) => (
              <CommandItem
                ...
                onSelect={(currentLabel) => {
                  const selectedItem = options.find((odon) => odon.label === currentLabel);
