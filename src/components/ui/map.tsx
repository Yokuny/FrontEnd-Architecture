// Mapa Default - Preview example em:
...
function MapLayerGroup({ name, disabled, ...props }: LayerGroupProps & MapLayerGroupOption & { ref?: Ref<LayerGroup> }) {
...
function MapFeatureGroup({ name, disabled, ...props }: LayerGroupProps & MapLayerGroupOption & { ref?: Ref<FeatureGroup> }) {
  const context = useMapLayersContext();
...
function MapZoomControl({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <ButtonGroup orientation="vertical" aria-label="Zoom controls" className={cn('absolute top-1 left-1 z-1000 h-fit', className)} {...props}>
      <Button
        type="button"
        size="icon-sm"
        variant="secondary"
...
function MapDrawDelete() {
  ...
  return (
    <MapDrawActionButton drawAction="delete" controlRef={drawContext.deleteControlRef} createDrawTool={(L, map, featureGroup) => new L.EditToolbar.Delete(map, { featureGroup })}>
      <Trash2Icon />
    </MapDrawActionButton>
  );
}
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const init = async () => {
        ...
        const L_obj = (leaflet as any).default || leaflet;
        window.L = L_obj;
