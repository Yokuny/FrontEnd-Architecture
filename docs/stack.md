...
## Back-End (`DentalEase/DentalEase-BackEnd/`)
...
│
└── DentalEase/DentalEase-BackEnd/  # API REST
    ├── Route → Controller → Service → Repository → MongoDB
    ├── Validation   → Zod middlewares
    ├── Events       → Domain events (financial, reminder, birthday)
    └── Auth         → JWT + role-based (admin, professional, assistant, guest)
```
