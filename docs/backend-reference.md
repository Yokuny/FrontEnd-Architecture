...
O back-end fica em `../DentalEase/DentalEase-BackEnd/`. Node.js + Express + MongoDB + Mongoose + Zod + Biome.
...
| `/patient` | Pacientes | CRUD + odontogram embedded + image |
| `/schedule` | Agendamentos | CRUD + status + time + scheduled-patient (use-case) |
| `/financial` | Financeiro | CRUD + status |
| `/odontogram` | Odontogramas | CRUD |
| `/clinic` | Clínica | Configurações da clínica |
| `/user` | Usuários | Gestão de usuários |
| `/procedure` | Procedimentos | Catálogo de procedimentos |
| `/reminder` | Lembretes | Gestão de lembretes |
| `/s3` | Upload | Upload de arquivos via AWS S3 |
| `/passkey` | Passkey | Login via código único |
...
```
validToken → clinicAssignmentCheck → [isAssistant | isAdmin | isProfessional]
```

- `isAssistant`: Acesso leitura/escrita (assistente+)
- `isAdmin`: Acesso total (apenas admin)
- `isProfessional`: Acesso de profissional
...
| Tipo | Descrição |
|------|-----------|
| `Patient` / `PartialPatient` / `FullPatient` | Paciente (com relações embedded) |
| `Schedule` | Agendamento |
| `Financial` | Registro financeiro |
| `Odontogram` | Odontograma |
| `Procedure` | Procedimento odontológico |
| `Reminder` | Lembrete |
| `ClinicUser` | Contexto do usuário autenticado |
| `Role` | `"admin" \| "professional" \| "assistant" \| "guest"` |
