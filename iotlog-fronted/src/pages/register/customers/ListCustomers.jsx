...
const ListCustomers = (props) => {
  ...
  const hasPermissionAdd = props.items?.some((x) => x === "/customer-add");
    ...
    if (
      props.itemsByEnterprise?.some(
        (x) =>
          ...
          x.paths?.includes("/customer-add")
      )
    ) {
      itemsMenu.push({
        icon: "edit-outline",
        title: intl.formatMessage({ id: "edit" }),
        link: { to: `/customer-add?id=${item.id}` },
      });
    }
        ...
        <ItemRow color={item?.color} style={{ flexWrap: "wrap" }}>
          <ColCenter breakPoint={{ md: 1 }}>
            <div className="flex-row-center">
              <IconRounded colorTextTheme={"colorPrimary600"}>
                <EvaIcon
                  name={"person-outline"}
                  options={{
                    fill: "#fff",
...
export default connect(mapStateToProps, undefined)(ListCustomers);
