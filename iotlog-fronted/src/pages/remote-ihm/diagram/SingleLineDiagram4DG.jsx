...
const SingleLineDiagram4DG = (props) => {
  ...
  return (
    <DiagramContainer>
      {props.data?.typeIHM === TYPE_DIAGRAM.UNIFILAR_4DG &&
        <>
          <GeneratorContainer>
            <IHMInfo
              infoList={data.info}
          ...
          <SensorContainer>
            {data.thrusters?.sort((a, b) => a.index - b.index).map((thruster) => {
              if (thruster.index === 2)
                return (<>
                  <div style={{ flexGrow: 1, maxWidth: '100px' }}></div>
                  <Sensor
                    idMachine={props.filter.machine?.value}
...
export default connect(mapStateToProps, mapDispatchToProps)(SingleLineDiagram4DG);