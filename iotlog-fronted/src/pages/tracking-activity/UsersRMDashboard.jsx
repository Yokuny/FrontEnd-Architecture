...
const UsersRMDashboard = (props) => {
  ...
  const getData = (periodFilter, idUsers, idUsersNotIncluded, idEnterprise) => {
    ...
    Fetch.get(`/tracking/usersrm?${queryPaths.join(`&`)}`)
      .then((res) => {
        setData(res.data);
        setIsLoading(false);
      })
  ...
  const options = {
    ...
    colors: [theme.colorDanger700],
    title: {
      text: `${intl.formatMessage({ id: "users" })} RM`,
      style: {
        foreColor: theme.textBasicColor,
...
export default connect(undefined, mapDispatchToProps)(UsersRMDashboard);
