...
const MachinesMostActionsWhatsappDashboard = (props) => {
  ...
  const getData = (
    ...
    Fetch.get(
      `/tracking/whatsappmostaccess?${queryPaths.join(`&`)}`
    )
      .then((res) => {
        setData(res.data);
        setIsLoading(false);
  ...
  const options = {
    ...
    labels: dataSorted?.length ? dataSorted?.map((x) => x.machine?.name ?? 'UNKOW') : [],
    responsive: [
      {
        breakpoint: 480,
        options: {
    ...
    title: {
      text: `WhatsApp${props.name && ` - ${props.name}`}`,
      style: {
...
const mapStateToProps = (state) => ({
  usersFilter: state.statistics.userFilterWhatsapp ? [state.statistics.userFilterWhatsapp] : [],
  name: state.statistics.userFilterWhatsappName,
});

export default connect(mapStateToProps, undefined)(MachinesMostActionsWhatsappDashboard);
