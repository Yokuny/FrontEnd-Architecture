...
const DevicesDashboard = (props) => {
  ...
  const getData = (
    ...
    Fetch.get(
      `/tracking/devices?${queryPaths.join(`&`)}`
    )
      .then((res) => {
        setIsLoading(false);
      ...
      <LoadingCard isLoading={isLoading}>
        <Row between style={{ margin: 0 }} className="pr-2 pl-2">
          <TextSpan apparence="s1">
            <FormattedMessage id="device" />{`${props.name && ` - ${props.name}`}`}
          </TextSpan>
          <TextSpan apparence="s1">
        ...
        <CardContent className="mt-1">
          {dataSorted?.map((browser, index) => (
            <Item key={`de-${index}`}>
              <Column>
                <TextSpan apparence="s2">
                  {`${browser?.info?.osName ?? browser?.info?.os} ${
                    browser?.info?.osVersion ?? ""
                  }`}
                </TextSpan>
                <TextSpan apparence="p3">{browser?.info?.browserName}</TextSpan>
              </Column>
              <TextSpan apparence="p1">{browser?.total}</TextSpan>
            </Item>
          ))}
        </CardContent>
      </LoadingCard>
...
export default connect(mapStateToProps, undefined)(DevicesDashboard);
