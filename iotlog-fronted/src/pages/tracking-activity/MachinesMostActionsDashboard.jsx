...
const MachinesMostActionsDashboard = (props) => {
  ...
  const getData = (
    ...
    Fetch.get(
      `/tracking/fleet-machine-details?${queryPaths.join(`&`)}`
    )
      .then((res) => {
        setData(res.data);
        setIsLoading(false);
  ...
  const options = {
    ...
    labels: dataSorted?.length ? dataSorted?.map((x) => x.machine ?? 'UNKOW') : [],
    responsive: [
      {
        breakpoint: 480,
        options: {
    ...
    title: {
      text: `Itens fleet${props.name && ` - ${props.name}`}`,
      style: {
...
export default connect(mapStateToProps, undefined)(MachinesMostActionsDashboard);
