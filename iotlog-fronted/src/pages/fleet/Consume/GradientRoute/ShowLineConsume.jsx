...
import { isValueValid, sortByDistance } from "../../../../components/Utils";
...
export const ShowLineConsume = (props) => {
      ...
      gradientRef.current.on("mouseover", function (e) {
        if (isValueValid(e.latlng.lat) && isValueValid(e.latlng.lng)) {
          ...
          setItemDetails({
            ...
            data: sortByDistance(dataList, e.latlng)[0]
          });
        }
      });
        ...
        <ContentDiv x={itemDetails.mousePoint.x} y={itemDetails.mousePoint.y}>
          <Row>
            <TextSpan apparence="s1">
              {itemDetails?.data?.consume?.toFixed(1)?.replace(".", ",") || " - "}
            </TextSpan>
          ...
          <Row>
            <TextSpan apparence="c3">
              {moment(itemDetails.data.date).format("DD MMM")}
            </TextSpan>
          </Row>
          <Row>
            <TextSpan apparence="s3">
              {moment(itemDetails.data.date).format("HH:mm")}
            </TextSpan>
          </Row>
        </ContentDiv>
      )}
