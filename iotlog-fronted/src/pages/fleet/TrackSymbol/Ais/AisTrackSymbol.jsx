...
export class AISTrackSymbol
  ...
  static DEFAULT_SILHOUETTE_SHAPE_POINTS = [[1, 0.5], [0.75, 1], [0, 1], [0, 0], [0.75, 0]];
  ...
  setShipStaticData(shipStaticData) {
    ...
    this.setStyle({
      color: shipStaticData?.color || shipType.color,
      fill: true,
      fillOpacity: 1.0,
      fillColor: shipStaticData.fullColor
      ? shipStaticData?.color
      : `${shipStaticData?.color}99` || shipType.fillColor,
    });
    // this.bindPopup(this._getPopupContent(this._positionReport, this._shipStaticData));
  ...
  static _getShapeOptions(leaderTime, minZoomLevel, size,
    shipStaticData) {
    const shapeOptions = {
