(function () {
	...
	let Renderer = window.L.Canvas.extend({
		_initContainer: function () {
			window.L.Canvas.prototype._initContainer.call(this);
			this._hotline = new Hotline(this._container);
		},

		_update: function () {
			window.L.Canvas.prototype._update.call(this);
	...
	let renderer = function (options) {
		return window.L.Browser.canvas ? new Renderer(options) : null;
	};
	...
	let Util = {
		...
		clipSegment: function (a, b, bounds, useLastCode, round) {
			var codeA = useLastCode ? this._lastCode : window.L.LineUtil._getBitCode(a, bounds),
					codeB = window.L.LineUtil._getBitCode(b, bounds),
					codeOut, p, newCode;
			...
			while (true) {
				...
				// other cases
					...
					codeOut = codeA || codeB;
					p = window.L.LineUtil._getEdgeIntersection(a, b, codeOut, bounds, round);
					newCode = window.L.LineUtil._getBitCode(p, bounds);
	...
	window.L.Hotline = window.L.Polyline.extend({
		statics: {
			Renderer: Renderer,
			renderer: renderer
		},
		...
		_clickTolerance: function () {
			return this.options.weight / 2 + this.options.outlineWidth + (window.L.Browser.touch ? 10 : 0);
		}
	});

	window.L.hotline = function (latlngs, options) {
		return new window.L.Hotline(latlngs, options);
	};
})();
