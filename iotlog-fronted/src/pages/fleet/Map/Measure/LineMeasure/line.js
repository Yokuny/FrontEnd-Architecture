(function () {
  window.L.Control.LinearMeasurement = window.L.Control.extend({
    options: {
      position: "topleft",
    ...
    onAdd: function (map) {
      var container = window.L.DomUtil.create(
        ...
        link = window.L.DomUtil.create("a", "icon-ruler", container),
      ...
      window.L.DomEvent.on(link, "click", window.L.DomEvent.stop).on(
        ...
        "click",
        function () {
          if (window.L.DomUtil.hasClass(link, "icon-active")) {
            me.resetRuler(!!me.mainLayer);
            window.L.DomUtil.removeClass(link, "icon-active");
            window.L.DomUtil.removeClass(map_container, "ruler-map");
          } else {
            me.initRuler();
            window.L.DomUtil.addClass(link, "icon-active");
            window.L.DomUtil.addClass(map_container, "ruler-map");
          }
        }
      );
    ...
    initRuler: function () {
      ...
      this.mainLayer = window.L.featureGroup();
      ...
      map.on("contextmenu", this.doubleClick, this);
    ...
    initLayer: function () {
      this.layer = window.L.featureGroup();
    ...
    resetRuler: function (resetLayer) {
      ...
      if (resetLayer) {
        ...
        map.off("contextmenu", this.doubleClick, this);
      ...
      if (this.options.unitSystem === "nauticalMiles") {
        this.UNIT_CONV = 1852;
        this.SUB_UNIT_CONV = 1;
        this.UNIT = "nm";
        this.SUB_UNIT = "nm";
      }
    ...
    renderCircle: function (latLng, radius, layer, type, label) {
      ...
      var a =
          this.prevLatlng && this._map?.latLngToContainerPoint
      ...
      let p_latLng = this._map?.containerPointToLatLng && this._map.containerPointToLatLng(b);
      ...
      if (p_latLng && label) {
        ...
        options.icon = cicon;
        options.marker = window.L.marker(p_latLng, {
          icon: cicon,
          type: type,
        }).addTo(layer);
        options.label = label;
      ...
      var circle = window.L.circleMarker(latLng, options);
    ...
    renderMultiPolyline: function (latLngs, dashArray, layer) {
      ...
      if (window.L.version.startsWith("0")) {
        multi = window.L.multiPolyline(latLngs, {
          color: this.options.color,
          weight: 2,
          opacity: 1,
          dashArray: dashArray,
        ...
        multi = window.L.polyline(latLngs, {
          color: this.options.color,
          weight: 2,
          opacity: 1,
          dashArray: dashArray,
    ...
    hasClass: function (target, classes) {
      var fn = window.L.DomUtil.hasClass;
    ...
    getMouseMoveHandler: function (e) {
      ...
      if (this.prevLatlng) {
        ...
        var a = this.prevLatlng
            ? this._map.latLngToContainerPoint(this.prevLatlng)
            : null,
          b = this._map?.latLngToContainerPoint && this._map.latLngToContainerPoint(latLng);
        ...
        if (!this.total) {
          this.totalIcon = window.L.divIcon({
            className: "total-popup",
            html: html,
          });

          this.total = window.L.marker(e.latlng, {
            icon: this.totalIcon,
            clickable: true,
          }).addTo(this.layer);
        } else {
    ...
    getDblClickHandler: function (e) {
      ...
      var fireSelected = function (e) {
        if (window.L.DomUtil.hasClass(e.originalEvent.target, "close")) {
