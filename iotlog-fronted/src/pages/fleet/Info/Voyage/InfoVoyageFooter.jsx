...
const InfoVoyageFooter = (props) => {
  ...
  const getData = (idMachine) => {
    setIsLoading(true);
    Fetch.get(`/travel/${idMachine}/last`
    )
      .then((response) => {
          ...
          : <>
            <Row className="m-0">
              ...
              <Col breakPoint={{ md: 8 }} className="mb-2">
                ...
                {data?.itinerary?.map((x, i) =>
                  <Row key={i}>
                    <Col breakPoint={{ md: 4 }} className="mb-1">
                      <TextSpan apparence="s2">
                        {`${i + 1}# ${x?.where}`}
                      </TextSpan>
                    </Col>
                    <Col breakPoint={{ md: 4 }} className="mb-1">
                      <TextSpan apparence="s2">
                        {x?.atd ? moment(x?.atd).format('DD MMM, HH:mm') : '-'}
                      </TextSpan>
                    </Col>
                     <Col breakPoint={{ md: 2 }} className="mb-1">
                      {x.load?.map((l, j) =>
                      <Row key={`${i}-${j}`} className="m-0">
                        <TextSpan apparence="s2">
                          {l?.description}
                        </TextSpan>
                        </Row>
...
export default InfoVoyageFooter;
