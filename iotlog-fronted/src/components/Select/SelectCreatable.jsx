...
const customStyle = ({ theme, status, shape, size }) => {
  ...
  return {
    control: (base, state) => {
      ...
      const disabled = isDisabled
        ? {
            ...
            backgroundColor: getThemeKey(
              `selectOutline${status}DisabledBackgroundColor`
            ),
            borderColor: getThemeKey(
              `selectOutline${status}DisabledBorderColor`
            ),
          }
        : {};
      return {
        ...
        "&:focus": {
          backgroundColor: getThemeKey(
            `selectOutline${status}FocusBackgroundColor`
          ),
          borderColor: getThemeKey(`selectOutline${status}FocusBorderColor`),
        },
        "&:hover": {
          backgroundColor: getThemeKey(
            `selectOutline${status}HoverBackgroundColor`
          ),
    ...
    multiValueRemove: (base) => {
      const borderTop =
        theme.dir === "rtl" ? "borderTopLeftRadius" : "borderTopRightRadius";
      const borderBottom =
        theme.dir === "rtl"
    ...
    option: (base, state) => {
      const optionStyle = state.isDisabled
        ...
        ? {
            ...
            "&:hover": {
              backgroundColor: getThemeKey(
                "selectOptionSelectedHoverBackgroundColor"
              ),
        ...
        : {
            ...
            color: state.isFocused
              ? getThemeKey("selectOptionFocusTextColor")
              : getThemeKey("selectOptionTextColor"),
            "&:hover": {
...
const SelectCreatable = (props) => {
  const { messageCreateId } = props;
  const intl = useIntl();
  return (
    <SelectMainWithTheme
      formatCreateLabel={(value) =>
        `${intl.formatMessage({ id: messageCreateId || "create" })} "${value}"`
      }
      {...props}
    />
  );
