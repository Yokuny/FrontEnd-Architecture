...
const SelectSignalCreateable = ({
  ...
  React.useEffect(() => {
    if (idMachine && sensorId && !sensorNew) {
      getData();
  ...
  const getData = () => {
    setIsLoading(true);
    Fetch.get(`/machine/sensor/signals?id=${idMachine}&sensorId=${sensorId}`)
      .then((response) => {
...
export default SelectSignalCreateable;
