...
export const FilterDataOverlay = (props) => {
  const { onApply, isLoading, title, renderFooter } = props;
  ...
  React.useLayoutEffect(() => {
    const options = getOptionsInterval(dateInit, dateEnd);
  ...
  const getOptionsInterval = (dateInitInternal, dateEndInternal) => {
    if (
      moment(dateEndInternal).diff(moment(dateInitInternal), "days") === 0 &&
      ...
      moment(dateEndInternal).diff(moment(dateInitInternal), "days") <= 1
    ) {
      return OPTIONS_DEFAULT.filter((x) => x.value >= 1);
    } else if (
      moment(dateEndInternal).diff(moment(dateInitInternal), "days") <= 2
    ) {
      return OPTIONS_DEFAULT.filter((x) => x.value >= 2);
    } else if (
      moment(dateEndInternal).diff(moment(dateInitInternal), "days") <= 3
    ) {
      return OPTIONS_DEFAULT.filter((x) => x.value >= 5);
    } else if (
      moment(dateEndInternal).diff(moment(dateInitInternal), "days") <= 5
    ) {
      return OPTIONS_DEFAULT.filter((x) => x.value >= 10);
    } else if (
      moment(dateEndInternal).diff(moment(dateInitInternal), "days") < 16
    ) {
  ...
  return (
    <>
      <Overlay
        ...
        trigger="click"
        placement="top"
        positionOverlay={{ top: 10, left: 45 }}
        showControlOutside={props.isShowFilter}
        controlOutside
